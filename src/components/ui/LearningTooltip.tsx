...
interface LearningTooltipProps {
  ...
  /** @deprecated Positioning is automatic — anchored to the orange indicator dot */
  position?: string
  className?: string
}
...
export function LearningTooltip({
  ...
  useEffect(() => {
    ...
    // offsetWidth/offsetHeight are unaffected by CSS transform animations (zoom-in-95)
    ...
    // On mobile: full-width strip below the trigger
    if (viewportW < 640) {
      setTooltipPosition({ top: triggerRect.bottom + padding, left: padding })
      return
    }

    // Anchor the tooltip's top-right corner to the orange dot position.
    // Orange dot: absolute -top-1 -right-1  →  right = triggerRect.right + 4, top = triggerRect.top - 4
    let top = triggerRect.top - 4
    let left = triggerRect.right + 4 - tooltipW

    // If the tooltip goes below the viewport, flip it upward (anchor bottom-right to dot instead)
    if (top + tooltipH > viewportH - padding) {
      top = triggerRect.top + 4 - tooltipH
    }
    ...
    >
      <div className="bg-dark-800 border border-dark-600 rounded-xl shadow-2xl shadow-black/50 overflow-hidden">
        ...
        <div className="p-4 space-y-3 max-h-[50vh] overflow-y-auto overscroll-contain">
          <p className="text-sm text-gray-300 leading-relaxed">{content.descricao}</p>
          ...
          {content.comoFunciona && (
            <div className="bg-dark-700/50 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-400 leading-relaxed">{content.comoFunciona}</p>
            </div>
          )}
          ...
          {content.comoCalculado && (
            <div className="bg-dark-700/50 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-semibold text-purple-400 uppercase tracking-wide">Como é calculado</span>
              </div>
          ...
          {content.exemplo && (
            <div className="bg-dark-700/50 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-400 leading-relaxed italic">"{content.exemplo}"</p>
            </div>
          )}
          ...
          {content.porqueImportante && (
            <div className="bg-dark-700/50 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-400 leading-relaxed">{content.porqueImportante}</p>
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      {/* Render via Portal so z-index is never trapped in a parent stacking context */}
    ...
    >
      <div className="bg-dark-800 border border-dark-600 rounded-lg shadow-xl shadow-black/50 overflow-hidden">
        ...
        <div className="p-3 space-y-2 max-h-[40vh] overflow-y-auto overscroll-contain">
          <p className="text-xs text-gray-300 leading-relaxed">{content.descricao}</p>
          {content.dicaPratica && (
